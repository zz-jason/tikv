...
impl<T, C> Store<T, C> {
    ...
    fn init(&mut self) -> Result<()> {
        ...
        let lsm_engine = self.lsm_engine.clone();
        ...
        if !wb.is_empty() {
            self.lsm_engine.write(wb).unwrap();
        }
    ...
    //        }
    //
    //        peer_storage::clear_meta(&self.lsm_engine, &self.blob_engine, wb, region.get_id()).unwrap();
...
impl<T: Transport, C: PdClient> Store<T, C> {
    pub fn run(&mut self, event_loop: &mut EventLoop<Self>) -> Result<()> {
        ...
        let split_check_runner = SplitCheckRunner::new(self.lsm_engine.clone(),
                                                       self.sendch.clone(),
                                                       self.cfg.region_max_size,
                                                       self.cfg.region_split_size);
        box_try!(self.split_check_worker.start(split_check_runner));

        let runner = RegionRunner::new(self.lsm_engine.clone(),
                                       self.snap_mgr.clone(),
        ...
        let compact_runner = CompactRunner::new(self.lsm_engine.clone());
    ...
    fn check_msg(&mut self, msg: &RaftMessage) -> Result<bool> {
        ...
        if let Some(local_state) = try!(self.lsm_engine.get_msg::<RegionLocalState>(&state_key)) {
            if local_state.get_state() != PeerState::Tombstone {
    ...
    fn on_raft_ready(&mut self) {
        ...
        if !wb.is_empty() {
            ...
            self.lsm_engine.write_opt(wb, &write_opts).unwrap_or_else(|e| {
    ...
    fn on_ready_compact_log(&mut self,
        ...
        let task = RaftlogGcTask {
            engine: peer.get_store().get_lsm_engine().clone(),
    ...
    fn store_heartbeat_pd(&mut self) {
        ...
        let disk_stats = match fs2::statvfs(self.lsm_engine.path()) {
            Err(e) => {
                error!("{} get disk stat for rocksdb {} failed: {}",
                       self.tag,
                       self.lsm_engine.path(),
                       e);
                return;
            }
            Ok(stats) => stats,
        ...
        let mut used_size = flush_engine_properties_and_get_used_size(self.lsm_engine.clone());
        ...
        let engine_total_bytes_written = self.lsm_engine
        ...
        let engine_total_keys_written = self.lsm_engine
    ...
    fn flush_engine_statistics(&mut self) {
        for t in ENGINE_TICKER_TYPES {
            let v = self.lsm_engine.get_statistics_ticker_count(*t);
            flush_engine_ticker_metrics(*t, v);
        }

        for t in ENGINE_HIST_TYPES {
            if let Some(v) = self.lsm_engine.get_statistics_histogram(*t) {
